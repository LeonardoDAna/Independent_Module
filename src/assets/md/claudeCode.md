...
与 3.5 的 Skill 不同，**Subagent** 是**主对话之外**的独立会话——这里我们用一个最朴素但完全可用的实现：**在主对话里手动派发一个 `claude -p` 无头（headless）子进程**，对指定文件做 **JS 变量声明 + CSS 类目声明** 的简单检查，结果落到日志或直接打印，**不打断当前对话**，也**不污染主上下文**。
